...
const AreaTip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background:'var(--sf)',border:'1px solid var(--bd2)',borderRadius:8,padding:'8px 12px',fontSize:12 }}>
      ...
      {payload.map(p => (
        <div key={p.name} style={{ color:p.color,fontFamily:"'DM Mono',monospace",marginBottom:2 }}>
...
function buildSuggestions({ fin, goals, salv, settings }) {
  const S = []
  const { netto, entrate, uscite } = fin
  const maxA = Math.round(netto * MAX_SAVINGS_RATIO)
  const buf = netto - maxA
  const stip = parseFloat(settings.stipendioNetto) || 0

  if (entrate > 0) {
    const ratio = netto / entrate
    if (ratio < 0.10) {
      S.push({ type:'danger', icon:'⚠', titolo:'Risparmio critico',
        testo:`Solo ${Math.round(ratio*100)}% risparmiato. Obiettivo minimo: 20%.` })
    } else if (ratio >= 0.30) {
      S.push({ type:'success', icon:'✓', titolo:'Ottimo tasso di risparmio',
        testo:`${Math.round(ratio*100)}% risparmiato. Suggerisco: max ${formatCurrency(maxA)} al risparmio, ${formatCurrency(buf)} liquidi.` })
    } else {
      S.push({ type:'info', icon:'→', titolo:'Ottimizza il risparmio',
        testo:`Tasso: ${Math.round(ratio*100)}%. Considera ${formatCurrency(maxA)} al risparmio (${Math.round(MAX_SAVINGS_RATIO*100)}%), tenendo ${formatCurrency(buf)} liquidi.` })
    }
  }

  if (settings.tredicesima && stip > 0) {
    const m = ['Gen','Feb','Mar','Apr','Mag','Giu','Lug','Ago','Set','Ott','Nov','Dic'][settings.meseTredicesima??11]
    S.push({ type:'info', icon:'🗓', titolo:`Tredicesima a ${m}`,
      testo:`~${formatCurrency(stip)} in arrivo. Momento ideale per obiettivi ad alta priorità.` })
  }
  if (settings.quattordicesima && stip > 0) {
    const m = ['Gen','Feb','Mar','Apr','Mag','Giu','Lug','Ago','Set','Ott','Nov','Dic'][settings.meseQuattordicesima??6]
    S.push({ type:'info', icon:'🗓', titolo:`Quattordicesima a ${m}`,
      testo:`~${formatCurrency(stip)} previsti. Pianifica l'allocazione in anticipo.` })
  }

  const fe = goals.find(g => g.nome.toLowerCase().includes('emergenza'))
  if (!fe && uscite > 0) {
    S.push({ type:'danger', icon:'!', titolo:'Fondo emergenza assente',
      testo:`Target minimo: ${formatCurrency(uscite*3)} (3 mesi spese), ideale ${formatCurrency(uscite*6)}.` })
  } else if (fe && uscite > 0 && fe.corrente < uscite * 3) {
    S.push({ type:'warning', icon:'△', titolo:'Fondo emergenza insufficiente',
      testo:`Copre ${Math.round(fe.corrente/Math.max(1,uscite))} mesi. Target: 3 mesi (${formatCurrency(uscite*3)}).` })
  }

  const ap = goals.filter(g => g.priorita==='alta' && g.corrente < g.target)
  if (ap.length > 0 && netto > 0) {
    const g = ap[0]
    const mesi = mesiAlTraguardo(g.corrente, g.target, maxA)
    S.push({ type:'action', icon:'⊕', titolo:`Focus: ${g.nome}`,
      testo:`Allocando ${formatCurrency(maxA)}/mese raggunti questo obiettivo in ~${mesi===Infinity?'∞':mesi} mesi.` })
  }

  if (salv > 500) {
    S.push({ type:'action', icon:'◈', titolo:'Considera un investimento',
      testo:`Hai ${formatCurrency(salv)} nel salvadanaio. Usa il simulatore per vedere la crescita composta.` })
  }
  return S.slice(0, 5)
}

// ── Projection based purely on salvadanaio balance + monthly contribution ──
function buildProiezione(salvadanaioLibero, contributoMensile, tasso = 0) {
  const r = tasso / 100 / 12
  return Array.from({ length: 12 }, (_, i) => {
    const n = i + 1
    const semplice = Math.round(salvadanaioLibero + contributoMensile * n)
    const investito = r === 0
      ? semplice
      : Math.round(salvadanaioLibero * Math.pow(1+r, n) + (contributoMensile * (Math.pow(1+r, n) - 1)) / r)
    return { mese: n, semplice, investito }
  })
}

const SCENARIOS = [
  { label:'Conservativo', tasso:2.5, color:'#3A7059' },
  { label:'Moderato',     tasso:5,   color:'#C46A3C' },
  { label:'Aggressivo',  tasso:8,   color:'#3A5F8A' },
]
...
export default function Risparmi() {
  ...
  // Projection state — purely based on salvadanaio + manual contribution
  const [contributo, setContributo] = useState(() => Math.max(0, Math.round(fin.netto * MAX_SAVINGS_RATIO)) || 100)
  ...
  const proiezione = useMemo(() =>
    buildProiezione(salvadanaioLibero, contributo, invOpen ? tasso : 0),
    [salvadanaioLibero, contributo, invOpen, tasso]
  )

  const suggestions = useMemo(() =>
    buildSuggestions({ fin, goals, salv: salvadanaioLibero, settings }),
    [fin.netto, fin.uscite, fin.entrate, goals.length, salvadanaioLibero,
     settings.tredicesima, settings.quattordicesima]
  )
  ...
  const handleRemoveGoal = g =>
    showConfirm(`Eliminare "${g.nome}"?`, () => removeGoal(g.id))
  ...
  return (
    <div style={{ padding:28, animation:'fadeUp .24s ease both' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, marginBottom:12 }}>
        {[
          ['TOTALE PATRIMONIO', totale>0?formatCurrency(totale):'—', 'var(--ac)', 'obiettivi + libero'],
          ['SALVADANAIO LIBERO', formatCurrency(salvadanaioLibero), undefined, 'non vincolato'],
          ['SALDO DISPONIBILE', formatCurrency(saldoDisp), saldoDisp>=0?undefined:'var(--rd)', fin.netto>0?`max ${formatCurrency(maxCons)} consigliato`:'da finanze'],
        ].map(([l,v,c,s],i) => (
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1.9fr 1fr', gap:12, alignItems:'start' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          ...
          <div className="card card-4">
            <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:12, gap:10 }}>
              <div>
                <div className="label-xs">evoluzione salvadanaio libero — 12 mesi</div>
                <div style={{ fontSize:11,color:'var(--t3)',marginTop:2 }}>
                  Partenza: <strong style={{ fontFamily:"'DM Mono',monospace",color:'var(--t1)' }}>{formatCurrency(salvadanaioLibero)}</strong> · versamenti mensili stimati
                </div>
              </div>
              <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
                <div style={{ display:'flex', alignItems:'center', gap:5 }}>
                  ...
                  <input type="number" value={contributo} min="0" step="50"
                    ...
                    style={{ width:72,padding:'4px 7px',border:'1px solid var(--bd2)',borderRadius:6,fontSize:13,background:'var(--bg)',color:'var(--t1)',fontFamily:"'DM Mono',monospace",outline:'none' }} />
                  <span style={{ fontSize:11,color:'var(--t2)' }}>€/mese</span>
                </div>
                <button className="tip-chip"
            ...
            {invOpen && (
              <div style={{ padding:'10px 14px', background:'var(--sf2)', borderRadius:9, marginBottom:12, border:'1px solid var(--bd)', animation:'slideDown .2s ease' }}>
                <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:8, flexWrap:'wrap' }}>
                  <span style={{ fontSize:12,fontWeight:600,color:'var(--t1)' }}>Simulazione rendimento</span>
                  ...
                  <div style={{ display:'flex', gap:5 }}>
                    {SCENARIOS.map(s => (
                      <button key={s.label} onClick={() => setTasso(s.tasso)}
                        style={{ padding:'3px 9px',border:`1px solid ${s.color}33`,borderRadius:6,background:tasso===s.tasso?s.color+'22':'transparent',color:s.color,fontSize:11,cursor:'pointer',fontFamily:"'DM Sans',sans-serif",transition:'all .13s' }}>
                        {s.label} {s.tasso}%
                      </button>
                    ))}
                  </div>
                  ...
                  {[3,6,12].map(n => {
                    ...
                    return (
                      <div key={n} style={{ padding:'8px 10px',background:'var(--bg)',borderRadius:8,border:'1px solid var(--bd)' }}>
                        <div style={{ fontSize:10,color:'var(--t3)',marginBottom:3,letterSpacing:'.06em' }}>{n} MESI</div>
                        <div style={{ fontSize:14,fontFamily:"'DM Mono',monospace",fontWeight:600,color:'var(--go)' }}>{formatCurrency(inv)}</div>
                        <div style={{ fontSize:10,color:'var(--t3)',marginTop:2 }}>+{formatCurrency(inv-semplice)} vs senza</div>
                      </div>
                    )
                  })}
                </div>
            ...
            <ResponsiveContainer width="100%" height={150}>
              <AreaChart data={proiezione} margin={{ top:4,right:4,bottom:0,left:0 }}>
                <defs>
                  <linearGradient id="gSalv" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="var(--ac)" stopOpacity={0.15}/>
                    <stop offset="100%" stopColor="var(--ac)" stopOpacity={0.01}/>
                  </linearGradient>
                  <linearGradient id="gInv" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="var(--go)" stopOpacity={0.18}/>
                    <stop offset="100%" stopColor="var(--go)" stopOpacity={0.01}/>
                  </linearGradient>
                </defs>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--bd)" />
                <XAxis dataKey="mese" tick={{ fontSize:11,fill:'var(--t2)',fontFamily:"'DM Mono'" }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fontSize:11,fill:'var(--t2)',fontFamily:"'DM Mono'" }} axisLine={false} tickLine={false}
                  tickFormatter={v=>`€${(v/1000).toFixed(0)}k`} width={36} />
                <Tooltip content={<AreaTip />} />
                <Area type="monotone" dataKey="semplice" stroke="var(--ac)" strokeWidth={1.5} fill="url(#gSalv)" name="Senza investimento" />
                {invOpen && <Area type="monotone" dataKey="investito" stroke="var(--go)" strokeWidth={1.5} fill="url(#gInv)" name={`Con ${tasso}%/anno`} />}
              </AreaChart>
            </ResponsiveContainer>
            ...
            <div style={{ display:'flex', gap:14, marginTop:8 }}>
              <div style={{ display:'flex', alignItems:'center', gap:5, fontSize:11, color:'var(--t2)' }}>
                <span style={{ width:14,height:2,background:'var(--ac)',display:'inline-block',borderRadius:1 }} />
                Accumulazione
              </div>
              {invOpen && (
                <div style={{ display:'flex', alignItems:'center', gap:5, fontSize:11, color:'var(--t2)' }}>
                  <span style={{ width:14,height:2,background:'var(--go)',display:'inline-block',borderRadius:1 }} />
                  Con {tasso}%/anno
                </div>
              )}
              <div style={{ marginLeft:'auto', fontSize:11, color:'var(--t3)', fontFamily:"'DM Mono',monospace" }}>
                A 12 mesi: <strong style={{ color:'var(--ac)' }}>{formatCurrency(proiezione[11]?.semplice||0)}</strong>
                {invOpen && <span style={{ color:'var(--go)',marginLeft:8 }}>+investimento: {formatCurrency(proiezione[11]?.investito||0)}</span>}
              </div>
            </div>
          </div>
          ...
          <div className="card card-5">
            ...
            <FormPanel open={goalFormOpen}>
              <input className="input-field" placeholder="Nome obiettivo" value={goalForm.nome}
                onChange={e => setGoalForm(f=>({...f,nome:e.target.value}))} />
              <InputRow>
                <input className="input-field" type="number" placeholder="Target €" value={goalForm.target}
                  onChange={e => setGoalForm(f=>({...f,target:e.target.value}))} />
                <input className="input-field" type="number" placeholder="Attuale €" value={goalForm.corrente}
                  onChange={e => setGoalForm(f=>({...f,corrente:e.target.value}))} />
              </InputRow>
              <InputRow>
                <select className="input-field" value={goalForm.priorita}
                  onChange={e => setGoalForm(f=>({...f,priorita:e.target.value}))}>
                  <option value="alta">Alta priorità</option>
                  <option value="media">Media priorità</option>
                  <option value="bassa">Bassa priorità</option>
                </select>
                <input className="input-field" type="date" value={goalForm.scadenza||''}
                  onChange={e => setGoalForm(f=>({...f,scadenza:e.target.value}))} title="Scadenza (opz.)" />
              </InputRow>
              <div style={{ fontSize:11,color:'var(--t3)' }}>📅 La scadenza sarà visualizzata nel calendario</div>
              <InputRow>
            ...
            {goals.length === 0
              ...
              : goals.map(g => {
                ...
                const cn = contributoNecessario ? contributoNecessario(g.id) : null
                  ...
                  <div key={g.id} style={{ marginBottom:14,paddingBottom:14,borderBottom:'1px solid var(--bd)' }}>
                    ...
                    <div style={{ display:'flex', justifyContent:'space-between', marginBottom:5 }}>
                      <span style={{ fontSize:11,color:'var(--t3)' }}>
                        {pct>=100?'✓ raggiunto':mesi===Infinity?'—':`~${mesi} mesi`}
                      </span>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          ...
          <div className="card card-2">
            ...
            <div style={{ fontSize:30,fontFamily:"'DM Mono',monospace",fontWeight:500,letterSpacing:'-.03em',color:'var(--ac)',marginBottom:4 }}>
              {formatCurrency(salvadanaioLibero)}
            </div>
            <div style={{ fontSize:11,color:'var(--t3)',marginBottom:14,fontFamily:"'DM Mono',monospace" }}>
              saldo disponibile: {formatCurrency(saldoDisp)}
            </div>
            ...
            <FormPanel open={depositFormOpen}>
              <div style={{ fontSize:11,color:'var(--t2)',lineHeight:1.6,padding:'5px 8px',background:'var(--sf2)',borderRadius:6 }}>
                Registrato come spesa (Risparmio) in Finanze.
              </div>
              <InputRow>
                <input className="input-field" type="number"
            ...
            {suggestions.length === 0 ? (
              <div style={{ textAlign:'center', padding:'20px 0' }}>
                ...
                <div style={{ fontSize:12,color:'var(--t3)',lineHeight:1.7 }}>
                  Inserisci dati in Finanze e Impostazioni per ricevere consigli calibrati.
                </div>
              </div>
            ) : suggestions.map((s,i) => {
              ...
              return (
                <div key={i} className="suggestion"
                  style={{ background:st.bg,border:`1px solid ${st.border}`,animationDelay:`${i*0.06}s`,transition:'transform .16s,box-shadow .16s' }}
